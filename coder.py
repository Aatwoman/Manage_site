...
// Safely grab the canvas from inside Streamlit's Shadow DOM
const svg = parentElement.querySelector("#canvas");
if (!svg) return;

// Always update the callback so it never goes stale across reruns
svg.__setStateValue = setStateValue;

// Early return guard: Prevents infinite event listener duplication 
